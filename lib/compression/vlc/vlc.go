...
func New() EncodeDecoder {
	return EncodeDecoder{}
}

func (_ EncodeDecoder) Encode(str string) []byte {
	str = prepareText(str)

	chunks := splitByChanks(encodeBin(str), chunkSize)

	return chunks.Bytes()
}

func (_ EncodeDecoder) Decode(encodedData []byte) string {
	bString := NewBinChunks(encodedData).Join()

	dTree := getEndodingTable().DecodingTree()
...
func getEndodingTable() encodingTable {
...
// change upper case letters to: ! + lower case letter
